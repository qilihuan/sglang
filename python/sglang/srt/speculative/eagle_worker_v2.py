...
def _sync_draft_sampling_across_tp(*tensors):
    ...
    path. Skipping the sync during capture means the replayed graph contains no
    sync at all. So use pynccl during capture, torch.distributed otherwise.
    """
    ...
    if get_is_capture_mode():
        ...
        for tensor in tensors:
            if tensor is not None and tensor.is_contiguous():
                pync.broadcast(tensor, src=0)
    else:
        for tensor in tensors:
